# Fotos pendientes — para subir y actualizar avisos

Feña deposita las fotos nuevas en `fotos/originales/`. Cuando estén, se aprueban, renombran por ID y se actualiza el ítem.

Actualizado: 2026-07-14

| ID | Ítem | Falta | Por qué |
|---|---|---|---|
| **EXT-001** | Comedor hierro + piedra (8 sillas) | Detalle del **óxido** de las sillas | §8.5 (defecto declarado). La general con las 8 sillas ya está (IMG_4475). |
| **LIV-006 / LIV-007** | Bibliotecas blancas | (opcional) Foto **vacía** de los estantes | Se venden vacías; las actuales (4641/4642) están ordenadas pero con libros. No bloquea. |
| **LIV-008** | 2 sillones blancos + puffs | Los **2 con sus puffs** juntos · detalle de **manchas** | Juego completo + §8.5 |
| **GEN-001** | Ventilador de pared Airolite | **Cambiar la foto** por una mejor | Pedido de Feña |

## Ítems NUEVOS sin publicar (falta foto/definición)
| Ítem | Falta |
|---|---|
| **EXT-003** — 2 reposeras de ratán | Foto de **las 2 juntas** (las actuales muestran 1) |
| **EXT-004** — pouf de ratán negro | Definir si se publica (hay foto: IMG_4485) |
| **Vitrina / vajillero blanco** provenzal | Confirmar **si se vende** + **foto propia** |

## Por fotografiar — ítems futuros (aún sin datos; Feña saca las fotos)

_Niños / dormitorios:_
- Camas de los niños
- Casa de muñecas
- Casa de LOLs
- Coche doble (cochecito)
- Mesa redonda de los niños con sillas
- Jugueteros de los niños (uno de madera + uno de melamina; la parte de arriba está en el cuarto de las niñas)
- Cajas para ordenar (del closet)

_Exterior:_
- Parrilla
- Malla de la piscina
- Reposeras (= **EXT-003**, foto de las 2 juntas)

_Cocina / electro:_
- Batidora
- Microondas
- Plancha (cerrada en caja, en un closet)

_Vajilla:_
- Juego de platos completo
- Juego de copas

_Otros:_
- Tablet (de dibujo / pintura)
- Sillón blanco (confirmar si es distinto del LIV-008)

> Se va completando a medida que avanzan los ítems.
